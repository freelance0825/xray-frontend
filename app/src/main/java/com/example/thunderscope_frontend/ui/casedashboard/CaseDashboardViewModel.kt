...
class CaseDashboardViewModel(private val repository: ThunderscopeRepository) : ViewModel() {
        ...
        viewModelScope.launch(Dispatchers.Main) {
            repository.getAllDoctors().collect { result ->
                when (result) {
                    ...
                    is Result.Success -> {
                        _isLoading.value = false
                        _doctorRecordsLiveData.value = result.data.toMutableList()
                    }
    ...
    class Factory(private val repository: ThunderscopeRepository) : ViewModelProvider.Factory {
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(CaseDashboardViewModel::class.java)) {
                return CaseDashboardViewModel(repository) as T
            }
